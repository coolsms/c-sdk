...
register_opt register_opt_init() {
	register_opt register_info = { "\0", "\0" };
	return register_info;
}

verify_opt verify_opt_init() {
	verify_opt verify_info = { "\0" };
	return verify_info;
}

delete_opt delete_opt_init() {
	delete_opt delete_info = { "\0" };
	return delete_info;

}

list_opt list_opt_init() {
	list_opt list_info = { "\0" };
	return list_info;
}

set_default_opt set_default_opt_init() {
	set_default_opt set_default_info = { "\0", "\0" };
	return set_default_info;

}

get_default_opt get_default_opt_init() {
	get_default_opt get_default_info = { "\0" };
...
response_struct _balance(const user_opt *u)
{
	...
	if (curl_process(options, "balance", "sms", GET, &output) == CURLE_OK)
		printf("\nSuccess!\n");
	else
		printf("\nError!\n");
	return output;
...
response_struct _verify(const user_opt *u, const verify_opt *v)
{
	char options[1024];
	response_struct output;
	...
	if (curl_process(options, "verify", "senderid", POST, &output) == CURLE_OK)
		printf("\nSuccess!\n");
	else
		printf("\nError!\n");
	return output;
...
response_struct _list(const user_opt *u, const list_opt *l) {
	char options[1024];
	response_struct output;

	sprintf(options, "api_key=%s&timestamp=%s&salt=%s&signature=%s&site_user=%s", 
		u->api_key, u->timestamp, u->salt, u->signature, l->site_user);

	if (curl_process(options, "list", "senderid", GET, &output) == CURLE_OK)
		printf("\nSuccess!\n");
	else
		printf("\nError!\n");
	return output;
...
response_struct _set_default(const user_opt *u, const set_default_opt *sd) {
	char options[1024];
	response_struct output;

	sprintf(options, "api_key=%s&timestamp=%s&salt=%s&signature=%s&handle_key=%s&site_user=%s", 
		u->api_key, u->timestamp, u->salt, u->signature, sd->handle_key, sd->site_user);

	if (curl_process(options, "set_default", "senderid", POST, &output) == CURLE_OK)
		printf("\nSuccess!\n");
	else
		printf("\nError!\n");
	return output;
...
int curl_process(char *options, char *path, char *group,int method, response_struct *output)
{
	CURL *curl;
	CURLcode res;
	char url[1024];
	...
	// method == 1 : POST, == 0 : GET
	if (method)
		sprintf(url, "%s/%s/%s/%s", "http://api.coolsms.co.kr/", group, VER, path);
	else
		sprintf(url, "%s/%s/%s/%s?%s", "http://api.coolsms.co.kr/", group, VER, path, options);
